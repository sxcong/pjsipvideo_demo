...
status_cb  m_pfStatusCB = NULL;
void*      m_pParamCB = NULL; 
...
static pj_bool_t log_on_rx_msg(pjsip_rx_data *rdata)//本地发出命令之后，收到对方消息，直到ACK
{
	...
	else
	{
		...
		PJ_LOG(1,(THIS_FILE, "      Received %s %s sdp", info,(msg->body ? "with" : "without")));
	}

		if (msg->body)
		{
			//pjmedia_sdp_session *sdp2 = create_sdp_from_string(pool, NULL);
			pjmedia_sdp_session *sdp;
			pj_str_t dup;
			pj_status_t status;

			pj_strdup2_with_null(app_config.pool, &dup, (const char*)(msg->body->data));
			status = pjmedia_sdp_parse(app_config.pool, dup.ptr, dup.slen, &sdp);
			...
			if (status == PJ_SUCCESS)
			{
				...
				for (int i = 0; i < sdp->media_count; i++)
				{
					pjmedia_sdp_media *m = sdp->media[i];
					nPort = m->desc.port ;
					//这两个是对方的媒体端口。收到后保存起来，直接收到ACK，开始Atach RTP接收通道。
				}
				*/

				pjmedia_sdp_media *m = sdp->media[sdp->media_count - 1];
				g_remote_port = m->desc.port ;

				pjmedia_sdp_conn *c = sdp->conn;
				if (c)
				{
					g_remote_ip = c->addr.ptr;
					int nPos = g_remote_ip.find("b");
					g_remote_ip = g_remote_ip.substr(0, nPos-1);
				}
...
//	string  rdata_info = (char*)(rdata->endpt_info.mod_data[0]);

	string log = rdata->msg_info.msg_buf;
//	string log2 = (char*)(rdata->tp_info.tp_data);
...
/*
	...
	evbuffer_add(pTemp, rdata->msg_info.msg_buf, rdata->msg_info.len);
	while(1)
	{
	...
	*/
	...
	if (m_pfStatusCB)
	{
		m_pfStatusCB(PJSUA_STATUS_INCOMING_CALL, &call_info,m_pParamCB);
	}
}
...
pj_data_cb m_pfDataCB = NULL;
void*      m_pDataCBParam = NULL; 

void set_data_callback(pj_data_cb cb, void* param)
{
	m_pfDataCB = cb;
	m_pDataCBParam = param;
}

register_status_cb  m_pfRegisterStatusCB = NULL;
void*      m_pRegisterParamCB = NULL; 



pjsip_log_cb m_pfLobCB = NULL;
void*  m_pLogParam = NULL; 

pjsip_msg_cb m_pfMsgCB = NULL;
void*  m_pMsgParam = NULL; 



static void LogCB(int level, const char *data, int len)
{
	if (m_pfLobCB)
	{
		m_pfLobCB(level, data, len, m_pLogParam);
...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
    ...
    if (call_info.state == PJSIP_INV_STATE_DISCONNECTED) {
		if (m_pfStatusCB)
		{
			m_pfStatusCB(PJSUA_STATUS_DISCONNECTED, &call_info,m_pParamCB);
		}
	...
	if ( call_info.state == PJSIP_INV_STATE_CONFIRMED) 
	{
		if (m_pfStatusCB)
		{
			m_pfStatusCB(PJSUA_STATUS_CONFIRMED, &call_info,m_pParamCB);
		}
	...
	if (call_info.state == PJSIP_INV_STATE_EARLY) {
		...
		if (m_pfStatusCB)
		{
			//m_pfStatusCB(PJSIP_INV_STATE_EARLY, &call_info,m_pParamCB);
		}
		...
		{
			//pDlg->m_dlgcall.UpdateCallStatus(PJSUA_STATUS_CALLING,&call_info);
			if (m_pfStatusCB)
			{
				m_pfStatusCB(PJSUA_STATUS_CALLING, &call_info,m_pParamCB);
			}
		}
...
PJ_DEF(const char*) pjmedia_type_name1(pjmedia_type t)
{
                                        ...
                                        pj_bool_t *has_error)
{
    ...
    PJ_LOG(4,(THIS_FILE, "Call %d media %d [type=%s], status is %s",
	      ci->id, mi, pjmedia_type_name1(ci->media[mi].type),
...
static void on_reg_state2(pjsua_acc_id acc_id, pjsua_reg_info *info)
{
	if (info != NULL)
	{
		if ((info->cbparam->status == 0) && (info->cbparam->code == 200))
		{
			//注册成功
			//pDlg->m_dlgaccount.UpdateListStatus(acc_id,TRUE);

			if (m_pfRegisterStatusCB)
			{
				m_pfRegisterStatusCB(acc_id, true, m_pRegisterParamCB);
			}
		}
	}
}
		     ...
		     const pj_str_t *mime_type, const pj_str_t *text)
{
	...
	if (m_pfMsgCB)
	{
		string szFrom = from->ptr;
		int nPos = szFrom.find("sip:");
		int nPos2 = szFrom.find("@", nPos);
		szFrom = szFrom.substr(nPos+4, nPos2-nPos-4);
		m_pfMsgCB(call_id, (char*)(szFrom.c_str()), text->ptr, text->slen, m_pMsgParam);
	}
}
...
pj_status_t app_init()
{
   ...
   // app_config.cfg.cb.on_reg_state = &on_reg_state;
	...
	status = pjsip_endpt_register_module(pjsua_get_pjsip_endpt(), &mod_msg_logger);
	...
	//pjsip_endpt_register_module( pjsua_get_pjsip_endpt(), &mod_simpleua);
	...
	pj_str_t id;
    status = pjsua_codec_set_priority(pj_cstr(&id, "speex/8000"),
				      (pj_uint8_t)0);
    status = pjsua_codec_set_priority(pj_cstr(&id, "speex/16000"),
				      (pj_uint8_t)0);
    status = pjsua_codec_set_priority(pj_cstr(&id, "iLBC/8000"),
				      (pj_uint8_t)0);
    status = pjsua_codec_set_priority(pj_cstr(&id, "GSM/8000"),
				      (pj_uint8_t)0);
    status = pjsua_codec_set_priority(pj_cstr(&id, "speex/32000"),
				      (pj_uint8_t)0);
    status = pjsua_codec_set_priority(pj_cstr(&id, "G722/16000"),
				      (pj_uint8_t)0);
    status = pjsua_codec_set_priority(pj_cstr(&id, "PCMA/8000"), (pj_uint8_t)PJMEDIA_CODEC_PRIO_NORMAL);

	//status = pjsua_codec_set_priority(pj_cstr(&id, "PCMA/8000"), (pj_uint8_t)0);
    
	
	status = pjsua_codec_set_priority(pj_cstr(&id, "PCMU/8000"),
				      (pj_uint8_t)0);

	status = pjsua_codec_set_priority(pj_cstr(&id, "H264/90000"),(pj_uint8_t)PJMEDIA_CODEC_PRIO_NORMAL);
...
int sip_register(const char* strUserName, const char* strPw, const char* strServer, int& acc_id)
{
	...
	char id[80], registrar[80], uname[80], passwd[30];
	sprintf(id, "sip:%s@%s", CStringA(strUserName), CStringA(strServer));
	cfg.id = pj_str(id);
	sprintf(registrar, "sip:%s", CStringA(strServer));
	cfg.reg_uri = pj_str(registrar);
	cfg.cred_count = 1;
	cfg.cred_info[0].realm = pj_str("*");//如果需要再次鉴权，需要改为*
	cfg.cred_info[0].scheme = pj_str("digest");
	sprintf(uname, "%s", CStringA(strUserName));
	cfg.cred_info[0].username = pj_str(uname);
	cfg.cred_info[0].data_type = PJSIP_CRED_DATA_PLAIN_PASSWD;
	sprintf(passwd, "%s", CStringA(strPw));
	cfg.cred_info[0].data = pj_str(passwd);

	if (m_pfLobCB)
	{
		m_pfLobCB(100, strUserName, strlen(strUserName), m_pLogParam);
		m_pfLobCB(100, strPw, strlen(strPw), m_pLogParam);
		m_pfLobCB(100, strServer, strlen(strServer), m_pLogParam);


		m_pfLobCB(101, cfg.id.ptr, cfg.id.slen, m_pLogParam);
		m_pfLobCB(101, cfg.reg_uri.ptr, cfg.reg_uri.slen, m_pLogParam);
		m_pfLobCB(101, cfg.cred_info[0].realm.ptr, cfg.cred_info[0].realm.slen, m_pLogParam);
		m_pfLobCB(101, cfg.cred_info[0].scheme.ptr, cfg.cred_info[0].scheme.slen, m_pLogParam);
		m_pfLobCB(101, cfg.cred_info[0].username.ptr, cfg.cred_info[0].username.slen, m_pLogParam);
		m_pfLobCB(101, cfg.cred_info[0].data.ptr, cfg.cred_info[0].data.slen, m_pLogParam);
	}
...
void sip_upregister(int acc_id)
{
	...
	{
		//AfxMessageBox(_T("invalid acc id!"));
	}
	else
	{
...
int sip_call(int acc_id, const char* strCalledNO, const char* strServer, const char* recfile)
{
	if (strServer == NULL)
		return -1;
	char id[80];
	sprintf(id, "sip:%s@%s", CStringA(strCalledNO), CStringA(strServer));
	...
	char body[128];
	msg_data.content_type = pj_str("application/text");
	...
	pj_status_t status = pjsua_call_make_call(acc_id, &uri, 0, NULL, &msg_data, &current_call);
	if (status == PJ_SUCCESS)
	{
	}
	return status;
...
void sip_answer(int callid, const char* recfile)
{
	pjsua_msg_data msg_data;
	pjsua_msg_data_init(&msg_data);
	pjsua_call_answer(callid, 200, NULL, &msg_data);
}
...
{
	m_pfRegisterStatusCB = cb;
	m_pRegisterParamCB = param;
}
...
int send_message(int callid, int accid, char* dest, char* server, const char* buff)
{
	if (accid < 0)
		return -1;
	pj_str_t content = pj_str((char*)buff);
	pj_str_t mine = pj_str("Application/KSLP");
	if (callid != -1)
	{		
		pjsua_call_send_im(callid, &mine, (const pj_str_t*)&content, NULL, NULL);
	}
	else 
	{
		char id[80];
		sprintf(id, "sip:%s@%s", CStringA(dest), CStringA(server));

		int id1 = current_acc;
		pj_str_t tmp_uri = pj_str(id);
		
		//pjsua_im_typing(current_acc, &tmp_uri, PJ_TRUE, NULL);
		pjsua_im_send(accid, &tmp_uri, &mine, (const pj_str_t*)&content, NULL, NULL);
		//pjsua_im_send(accid, &tmp_uri, NULL, (const pj_str_t*)&content, NULL, NULL);
	}
	return 0;
}
...
struct evbuffer *outbuf = NULL;
 ...
 static void on_rx_rtp(void *user_data, void *pkt, pj_ssize_t size)//收到RTP视频
{
	...
	evbuffer_add(outbuf, payload, payload_len);
	...
	if (hdr->m == 1)
	{
		if (m_pfDataCB && m_pDataCBParam)
		m_pfDataCB((const char*)(outbuf->buffer), outbuf->off, 0, 0, m_pDataCBParam);
...
int init_recv_rtp(const char* destip, int destport)
{
	if (!m_bInitMedia)
		return -1;
	...
	pj_str_t ip = pj_str((char*)destip);
	pj_uint16_t port = destport;

	int status = pj_sockaddr_in_init(&remote_addr, &ip, port);


	//pjmedia_vid_stream_info info;
	//pj_bzero(&info, sizeof(info));
	//pj_memcpy(&info.rem_addr, (pj_sockaddr_in*)&remote_addr, sizeof(pj_sockaddr_in));
...
pj_status_t init_local_rtp()
{
	...
	init_recv_rtp(g_remote_ip.c_str(), g_remote_port);

	return 0;
}

unsigned rtp_ts_len;
int frame_ts_len = 90000/15;
int total_len = 0;
...
void send_rtp(char* buff, int len)
{
	...
	total_len += frame_ts_len;
	rtp_ts_len = total_len;

	if (len > 1400)
	{
		int nBlockNum = len / BLOCK_SIZE;
		if (len % BLOCK_SIZE != 0)
		{
			nBlockNum++;
		}
		...
		{
			int poayload_size = BLOCK_SIZE;
			char* payload = buff + BLOCK_SIZE * i;

			if (len - BLOCK_SIZE * i < BLOCK_SIZE)
				poayload_size = len - BLOCK_SIZE * i;
			...
			status = pjmedia_rtp_encode_rtp(&video.out_sess,
												97,
												mark,
												poayload_size,
												rtp_ts_len,
			...
			hdr = (const pjmedia_rtp_hdr*)rtphdr;
			memcpy(packet, hdr, rtphdrlen);
			memcpy(packet + rtphdrlen, payload, poayload_size);

			status = pjmedia_transport_send_rtp(video.transport, packet, poayload_size + rtphdrlen);
	...
	else
	{
		...
		//stream->frame_ts_len = info->codec_info.clock_rate */15;

		//vfd->fps.num   = DEFAULT_H264_FPS_NUM;  //15
	   // vfd->fps.denum = DEFAULT_H264_FPS_DENUM;//1
		//param->clock_rate = 90000;

		int status = pjmedia_rtp_encode_rtp(&video.out_sess,
				97,
				1,
				len,
				rtp_ts_len,
		...
		hdr = (const pjmedia_rtp_hdr*)rtphdr;
		pj_memcpy(packet, hdr, rtphdrlen);
		pj_memcpy(packet + rtphdrlen, buff, len);

		status = pjmedia_transport_send_rtp(video.transport, packet, len+rtphdrlen);
	}
