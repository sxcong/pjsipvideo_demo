/* $Id: pjsua_app_config.c 4779 2014-03-06 01:00:11Z ming $ */
/*
...
static pj_bool_t pjsip_use_compact_form = PJSIP_ENCODE_SHORT_HNAME;
static pj_bool_t pjsip_include_allow_hdr_in_dlg = PJSIP_INCLUDE_ALLOW_HDR_IN_DLG;
static pj_bool_t pjmedia_add_rtpmap_for_static_pt = PJMEDIA_ADD_RTPMAP_FOR_STATIC_PT;
			    ...
			    int *app_argc, char ***app_argv)
{
    ...
    argv = (char**)pj_pool_calloc(pool, MAX_ARGS+1, sizeof(char*));
	...
	for (p = line; *p != '\0' && argc < MAX_ARGS; p++) {
	    ...
	    *p = '\0';
		...
		argv[argc] = (char*)pj_pool_alloc(pool, token_len + 1);
			      ...
			      pj_str_t *uri_to_call)
{
	...
	case OPT_ACCEPT_REDIRECT:
	    cfg->redir_op = (pjsip_redirect_op)my_atoi(pj_optarg);
...
#if defined(PJMEDIA_HAS_SRTP) && (PJMEDIA_HAS_SRTP != 0)
	case OPT_USE_SRTP:
	    app_config.cfg.use_srtp = (pjmedia_srtp_use)my_atoi(pj_optarg);
...
static void default_config()
{
    ...
    cfg->udp_cfg.port = g_nPort; //5060;

	
	
    pjsua_transport_config_default(&cfg->rtp_cfg);
	cfg->rtp_cfg.port = 4000;
	//cfg->rtp_cfg.port = g_nRtpPort;
    ...
    for (i=0; i<PJ_ARRAY_SIZE(cfg->acc_cfg); ++i)
	{
		...
		//cfg->acc_cfg[i].rtp_cfg.port = g_nRtpPort;
	}
